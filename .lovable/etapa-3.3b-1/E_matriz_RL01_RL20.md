# MATRIZ DE TESTES RL01-RL20 (E) — ETAPA 3.3B-1
...
RL07 | Múltiplas instâncias server | Todas devem ler o mesmo estado no Postgres
RL08 | Isolamento de Identificador | Cliente A bloqueado não afeta Cliente B
RL09 | Limite Global do Endpoint | Proteção contra ataques distribuídos (botnet)
...
RL13 | Privacidade de Logs | Zero IPs ou PII nos logs de rate limit
RL14 | Segurança de Secret | Falha se `AV_RATE_LIMIT_HASH_SECRET` for nulo
RL15 | Storage Indisponível | Aplicação da política Fail-Open logada
RL16 | Cleanup (Auto-expiração) | Buckets antigos não devem persistir no banco (TTL)
