-- A_patch_rate_limit.sql
-- ETAPA 3.3B-2A-R1 — HARDENING DA FUNÇÃO TOKEN BUCKET
...
BEGIN
    ...
    FOR v_spec IN SELECT * FROM jsonb_array_elements(p_specs) LOOP
        ...
        END IF;
        v_ttl_seconds := (v_spec->>'ttl_seconds')::NUMERIC; -- cast to numeric first to handle decimals if any, though we want integer
    ...
    -- 4. Garantir existência (UPSERT safely) - Apenas cria se não existir
    FOR v_spec IN SELECT * FROM jsonb_array_elements(p_specs) LOOP
        v_bucket_hash := v_spec->>'bucket_key_hash';
        v_scope := v_spec->>'scope';
        v_capacity := (v_spec->>'capacity')::NUMERIC;
    ...
    -- Re-ordenar hashes para evitar deadlock
